...
Functions keys are the following:
    ...
    ENTER       Run Z80 SNA TAP TRD BAS ROM SCL, view SCR
...
ENTER over a .Z80, .SNA, .TAP, .TRD, .BAS, .ROM or .SCL load and run it. ENTER
over a .SCR load screen. ENTER over a no extension file treat it as .BAS file.
...
ENTER over a file do the following:
    ...
    .SCL: attaches disk image to drive A and try to boot it. If boot is not
          possible, then runs TR-DOS Navigator (/sys/trdos54t.ko).
...
New NMI navigator works on esxDOS versions from v0.8.5 to v0.8.9
